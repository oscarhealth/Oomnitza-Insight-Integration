...
class BaseConnector(object):
    ...
    def finalize_processed_portion(self):
        # In test mode, do not notify Oomnitza at all
        if self.settings.get('__testmode__'):
            self.logger.info('Test mode enabled: skipping finalize_portion call')
            return
    ...
    def is_run_canceled(self) -> bool:
        # In test mode, never query Oomnitza for cancellation
        if self.settings.get('__testmode__'):
            return False
    ...
    def perform_sync_bulk_paginated(self, options):
        ...
        try:
            ...
            for index, item in enumerate(self._load_records(options)):
                ...
                if not self.settings.get('__testmode__') and self.is_run_canceled():
                    break
            ...
            if not self.settings.get('__testmode__'):
