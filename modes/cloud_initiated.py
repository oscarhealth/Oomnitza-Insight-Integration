...
def run_the_managed_sync(cloud_config, cmdline_args):
    ...
    try:
        ...
        LOG.info("Starting managed job id=%s name=%s section=%s", cloud_config.get("id"), cloud_config.get("name"), connector_section)
...
def cloud_initiated_upload(cmdline_args):
    ...
    while True:
        try:
            LOG.debug("Polling Oomnitza for managed connector jobs...")
        ...
        if not cloud_configs:
            LOG.info("No managed jobs available; polling again in 10 seconds")
        else:
            LOG.info("Received %d managed job(s)", len(cloud_configs))
